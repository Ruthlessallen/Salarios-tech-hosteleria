# -*- coding: utf-8 -*-
"""Página 3: Detalle completo del sector Tecnología (Información y comunicaciones).
Un único toggle de unidad (por defecto / €·mes) controla el KPI de salario y los
dos gráficos de la página a la vez."""
import sys
import os
...
dash.register_page(__name__, path="/detalle-tech", name="Tecnología")

SECTOR = dl.SECTOR_TECH
COLOR = dl.COLOR_TECH
COLOR_SOFT = dl.COLOR_TECH_SUAVE
...
layout = html.Div([
    html.Div([
        html.Div([
            html.H1("Información y comunicaciones (Tech)", className="page-title", style={"color": COLOR}),
            html.P("CNAE Sección J · datos reales INE. Ranking por Comunidad Autónoma: ver página Mapa.", className="page-subtitle"),
        ]),
        html.Div([
            dcc.RadioItems(
    ...
    html.Div([
        kpi("", "Salario medio 2024", "INE EAES · real", id_valor="kpi-salario-2024-tech"),
...
@callback(
    Output("g3-anual", "figure"),
    Output("kpi-salario-2024-tech", "children"),
    Input("toggle-unidad-pagina", "value"),
)
def actualizar_g3(unidad):
...
@callback(Output("g3-jornada", "figure"), Input("toggle-unidad-pagina", "value"))
def actualizar_g3_jornada(unidad):
    ...
    fig = go.Figure()
    colores = {"Ambas jornadas": "#7C2D12", "Jornada a tiempo completo": COLOR, "Jornada a tiempo parcial": "#FDBA74"}
    ...
    etiqueta_y = "€ / hora efectiva" if unidad == "defecto" else "€ / mes"
    fig.update_layout(**dict(dl.TEMA_PLOTLY, margin=dict(l=8, r=60, t=8, b=8)),
                       legend=dict(orientation="h", y=1.15, font=dict(size=9)),
